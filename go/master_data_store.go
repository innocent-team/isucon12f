...
func hookRefreshGacha() error {
	...
	resp, err = http.Post("http://isucon2:8080/gacha/refresh", "application/x-www-form-urlencoded", nil)
	if err != nil {
		return fmt.Errorf("isucon2 refresh error: %w", err)
	}
	if resp.StatusCode != http.StatusOK {
		return fmt.Errorf("isucon2 refresh error: %s", resp.Status)
	}
	defer resp.Body.Close()
